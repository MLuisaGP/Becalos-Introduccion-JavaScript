...
let comentariosStorage = JSON.parse(localStorage.getItem('comentarios-i2'))??[];
let imagenLiked = JSON.parse(localStorage.getItem('liked-i2'))??[];
let imagenShared = localStorage.getItem('shared-i2')??0;
...
botonSend.addEventListener("click",function(event){
    ...
    localStorage.setItem('comentarios-i2',JSON.stringify(comentariosStorage));
    showComents();
    actualizarCntAcciones();
})
...
btnLiked.addEventListener('click',function(){
    ...
    localStorage.setItem('liked-i2',JSON.stringify(imagenLiked))
    actualizarCntAcciones();
})
...
btnShared.addEventListener('click',function(){
    ...
    localStorage.setItem('shared-i2',JSON.stringify(imagenShared))
    actualizarCntAcciones();
})
...
function showComents(){
    ...
    comentariosStorage.forEach(comentario => {
        ...
        if(comentario.perfilImag===user.urlImg &&  comentario.nombre===user.nombre){
